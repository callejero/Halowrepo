...
def log(message):
  ...
  xbmc.log((u'### %s' % message).encode('utf-8'), level=xbmc.LOGDEBUG)
...
def show_search(show, languages):
  ...
  for language in languages:
    ...
    if language == 'English':
      ...
      if season < len(ENGLISH_SEASON_NAMES):
        temp_string  = '%s - %s' % (show['name'], ENGLISH_SEASON_NAMES[season])
...
def download(url):
  ...
  if content:
    ...
    for current_path, folders, files in os.walk(__temp__):
      for file_ in files:
        if os.path.splitext(file_)[1].lower() in SUBTITLE_EXTENSIONS:
...
if __name__ == '__main__':
  ...
  if parameters['action'] in ['search', 'manualsearch']:
    search_string      = parameters['searchstring'].decode('utf-8') if 'searchstring' in parameters else None
    movie_title        = xbmc.getInfoLabel('VideoPlayer.OriginalTitle').decode('utf-8') or None
    show_title         = xbmc.getInfoLabel('VideoPlayer.TVShowTitle').decode('utf-8') or None

    file_path          = urllib.unquote(xbmc.Player().getPlayingFile().decode('utf-8'))
    languages          = parameters['languages'].decode('utf-8').split(',') if 'languages' in parameters else []
    preferred_language = parameters['preferredlanguage'].decode('utf-8') if 'preferredlanguage' in parameters else None

    # See if we support at least one of the given languages
    if not any(language in SUBTITLE_LANGUAGES for language in languages):
      log('No supported languages given')

      # Fallback on preferred language if it is set and supported
      if preferred_language and preferred_language in SUBTITLE_LANGUAGES:
    ...
    # Show search
      ...
      results = show_search({
        'name'    : show_title,
        'title'   : xbmc.getInfoLabel('VideoPlayer.Title').decode('utf-8'),
    ...
    # Filename search
      ...
      filename = os.path.splitext(xbmc.getInfoLabel('VideoPlayer.Title').decode('utf-8'))[0]
    ...
    if results:
      ...
      for result in results:
        if any(name.lower() == result['name'].lower() for name in names):
          result['sync'] = True
        else:
          result['sync'] = False
