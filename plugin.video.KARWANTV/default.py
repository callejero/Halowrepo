...
def add_video_item(url, infolabels, img=''):
    urlx = url.replace('http://karwan.tv/live/','').replace('.php','')
    url = 'http://karwan.tv/live/' + urlx + '/' + urlx + '.php'

    if 'rtmp://' in url:
        url = url.replace('<playpath>',' playpath=')
        url = url + ' swfUrl=http://p.jwpcdn.com/6/11/jwplayer.flash.swf pageUrl=http://karwan.tv live=1'
    url = 'plugin://plugin.video.AKO/?playkurd=' + url + '***' + infolabels['title'] + '***' + img
